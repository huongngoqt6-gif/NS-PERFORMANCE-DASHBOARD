...
def load_data(file):
  ...
  raw_bu_col_m = pd.read_excel(
      xls, sheet_name="BU allocation", skiprows=3, header=None
  )
...
if os.path.exists(file_path):
  try:
    ...
    page = st.sidebar.radio(
        "Navigation", ["Overview", "Shipment volume", "FTE", "BU Allocation"]
    )

    if page == "Overview":
      st.markdown(
          '<div class="main-title">CSD Operations performance dashboard</div>',
          unsafe_allow_html=True,
      )
      st.header("Overview")
      ...
      with c4:
        cap_status = (
            hc_df["Capacity_Status"].mode()[0]
            if not hc_df["Capacity_Status"].empty
            else ""
        )
        st.markdown(
            f"""
      ...
      with c_chart1:
        hc_trend = (
            hc_df.groupby("Month")[["Required_HC_Total", "Actual_HC_Total"]]
            .mean()
            .reset_index()
        )
        fig1 = go.Figure()
      ...
      with c_chart2:
        sv_trend = (
            sv_df.groupby(["Month", "Office"])["Total"].sum().reset_index()
        )
        fig2 = px.bar(
            sv_trend,
            x="Month",
            ...
            color="Office",
            barmode="group",
            title="Total Volume Trend by Month & Office",
        )
        fig2.update_layout(
            autosize=True,
            height=350,
      ...
      st.markdown(
          "<div style='color: white; margin-top: 20px; font-style: italic;'>* Ghi"
          " chú: 1 FTE tương ứng với 8 tiếng x 95% hiệu suất x 22 ngày trong"
          " tháng.</div>",
          unsafe_allow_html=True,
      )
      ...
      with c_chart1:
        top_customers = (
            nsc_df.sort_values(by="Total", ascending=False).head(10)
        )
        fig1 = px.bar(
            top_customers,
            x="Total",
      ...
      with c_chart2:
        ...
        fig2 = px.bar(
            ...
            orientation="h",
            text="Volume",
            title="Top Volume by Transportation Mode",
        )
      ...
      with col_chart:
        ...
        hc_trend = (
            hc_df.groupby("Month")[
                ["Required_HC_Total", "Actual_HC_Total", "Approved_HC_Total"]
            ]
            .mean()
            .reset_index()
        )
      ...
      with col_table:
        ...
        def color_hc_status(val):
          if val == "Overload":
            return (
                "background-color: #ffcccc; color: #cc0000; font-weight:"
                " bold;"
            )
          elif val == "High load":
            return (
                "background-color: #ffe6cc; color: #cc6600; font-weight:"
                " bold;"
            )
          elif val == "Balanced":
            return (
                "background-color: #cce6ff; color: #0066cc; font-weight:"
                " bold;"
            )
          elif val == "Less load":
            return (
                "background-color: #d9ffcc; color: #2e8b57; font-weight:"
                " bold;"
            )
          return ""

        styled_hc_table = df_hc_table.style.map(
            color_hc_status, subset=["Status"]
        )
      ...
      def color_status(val):
        if val == "Overload":
          ...
          return (
              "background-color: #ffe6cc; color: #cc6600; font-weight: bold;"
          )
        elif val == "Balanced":
          return (
              "background-color: #cce6ff; color: #0066cc; font-weight: bold;"
          )
        elif val == "Less load":
          return (
              "background-color: #d9ffcc; color: #2e8b57; font-weight: bold;"
          )
        return ""
      ...
      with c_left:
        seg_data = (
            bu_df.groupby("Segment")["Pct_of_Network"].mean().reset_index()
        )
        ...
        st.dataframe(
            ...
            column_config={
                "Pct_of_Network": st.column_config.NumberColumn(
                    "Pct_of_Network", format="%.1f%%"
                )
            },
        )

      st.subheader("Details by Services")
      t_a, t_c, t_s, t_e = st.tabs([
          "Core Service (A)",
          "Ancillary Service (C)",
          "Supporting Activity (S)",
    ...
    st.error(
        f"Lỗi khi đọc file: {e}. Vui lòng kiểm tra lại cấu trúc file Excel."
    )
else:
  st.error(
      f"Không tìm thấy file '{file_path}'. Vui lòng đảm bảo file Excel đã được"
      " đưa lên cùng thư mục trên GitHub."
  )
